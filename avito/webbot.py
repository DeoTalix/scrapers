...
class Browser:
    ...
    **Constructor**
    ...
    def __init__(self, headless=False, proxy='', driverpath='.'):
        ...
        [setattr(self, function, getattr(self.driver, function)) for function in ['add_cookie', 'delete_all_cookies', 'delete_cookie', 'execute_script', 'execute_async_script', 'fullscreen_window', 'get_cookie', 'get_cookies', 'get_log', 'get_network_conditions', 'get_screenshot_as_base64', 'get_screenshot_as_file', 'get_screenshot_as_png',
                                                                                  'get_window_position', 'get_window_rect', 'get_window_size', 'maximize_window', 'minimize_window', 'implicitly_wait', 'quit', 'refresh', 'save_screenshot', 'set_network_conditions', 'set_page_load_timeout', 'set_script_timeout', 'set_window_position', 'set_window_rect', 'start_client', 'start_session', 'stop_client', 'switch_to_alert']]
    ...
    def set_timeout(self, i):
        '''Sets time to wait before throwing timeout error'''
        self.driver.set_page_load_timeout(i)
    ...
    def __find_element(self, text, tag, classname, id, number, css_selector, xpath, loose_match):
            ...
            for element in elementlist:
                ...
                try:
                    ...
                    add_to_init_text_matches_score(element.find_elements_by_xpath(
                        "../input[contains(translate(@id, '{}','{}'), '{}')]".format(text.upper(), text.lower(),  text.lower())), score - 5)
                    add_to_init_text_matches_score(
                        element.find_elements_by_xpath("/./preceding::input"),  score - 7)
                    element_fetch_helper(
                        ("//body//input[@name='{}']".format(possible_input_id)), score-6)
                    add_to_init_text_matches_score(
        ...
        def handle_input_tag():
            if(text):
                for test_attr in ['@value', '@placeholder', 'name', '@aria-label']:
                    ...
                    element_fetch_helper(
                        ("//body//input[contains( {}, '{}')]".format(test_attr, text)), score=37)
                ...
                find_input_element_for_label(self.driver.find_elements_by_xpath(
                    "//body//label[text()='{}']".format(text)), score=45)
                find_input_element_for_label(self.driver.find_elements_by_xpath(
                    "//body//label[contains(text(), '{}')]".format(text)), score=37)
                find_input_element_for_label(self.driver.find_elements_by_xpath(
                    "//body//label[contains(translate(text(), '{}', '{}'), '{}')]".format(text.upper(), text.lower(), text.lower())), score=33)
        ...
        def handle_button_or_link_tag(tagvar):
            if(text):
                ...
                element_fetch_helper(
                    ("//body//{}[contains(text(), '{}')]".format(tagvar, text)), score=37)
                element_fetch_helper(
                    ("//body//{}//*[contains(text(), '{}')]".format(tagvar, text)), score=37)

                element_fetch_helper(("//body//{}[contains(translate(text(), '{}', '{}'), '{}')]".format(
                    tagvar, text.upper(), text.lower(), text.lower())), score=33)
                element_fetch_helper(("//body//{}//*[contains(translate(text(), '{}', '{}'), '{}')]".format(
                    tagvar, text.upper(), text.lower(), text.lower())), score=33)
            else:
        ...
        def handle_loose_check():
            ...
            if(text):
                element_fetch_helper(
                    "//body//*[@value='{}']".format(text), score=30)
                element_fetch_helper(
                    "//body//*[text()='{}']".format(text), score=30)
        ...
        if not text and tag:
            ...
            element_fetch_helper(("//body//{0}[contains(translate(text(), '{1}', '{2}'), '{2}')]".format(
                tag, text.upper(), text.lower())), score=48)
        ...
        if(text.lower() in ['username', 'email', 'login'] and tag == 'input'):
            element_fetch_helper(
                '''//body//input[contains(translate(@name, 'USERNAME', 'username'), 'username') or contains(translate(@name, 'EMAIL', 'email'), 'email') or contains(translate(@name, 'LOGIN', 'login'), 'login') or contains(translate(@type, 'EMAIL', 'email'), 'email')]''', 53)
        ...
        if id:
            add_to_init_text_matches_score(
                self.driver.find_elements_by_id(id), 100)
        if classname:
            add_to_init_text_matches_score(
        ...
        if(not len(self.element_to_score.keys())):
            self.__set_error(
                'Element not found ! ', message="There is no element that matches your search criteria.")
            return []
    ...
    def __set_error(self, Exceptionerror, element=None, message=''):
        ...
        self.errors.append({'Exceptionerror': Exceptionerror,
    ...
    def switch_to_tab(self, number):
        ...
        '''
        assert number <= len(
            self.driver.window_handles) and number > 0, "Tab number must be less than or equal to the total number of tabs"

        self.driver.switch_to_window(self.driver.window_handles[number-1])

    def go_back(self):
        '''Go back to the previous URL.
        's same as clicking the back button in browser . 
        '''
        self.driver.back()
    ...
    def type_in(self, text, into='', clear=True, multiple=False,  tag='input', id='', classname='',  number=1, css_selector='', xpath='', loose_match=True):
        '''
...
if __name__ == '__main__':
    ...
    aton.type_in("Hello its me ", into="Search")
